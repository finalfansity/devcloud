  ...
  <div class="host-container">
    ...
    <div class="box-shadow">
      <el-table :data="hosts" style="width: 100%">
        ...
        <el-table-column prop="private_ip" label="内网IP">
          <template slot-scope="scope">
            {{ scope.row.private_ip }}
          </template>
        </el-table-column>
        <el-table-column prop="os_name" label="系统类型">
          <template slot-scope="scope">
            {{ scope.row.os_name }}
          </template>
        </el-table-column>
        ...
        <el-table-column prop="create_at" label="配置规格">
          <template slot-scope="scope">
            {{ scope.row.cpu }} / {{ scope.row.memory }}
          </template>
        </el-table-column>
        <el-table-column prop="expire_at" label="状态">
          <template slot-scope="scope">
            {{ scope.row.status }}
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center" width="180">
          <template slot-scope="scope">
            ...
            <el-button type="text" @click="jumpToMonitor(scope.row.instance_id)"
              >监控</el-button
            >
          </template>
        </el-table-column>
